...
"""
T17
Goes through a function and if a line is longer than 79 characters (not including the \n) then
adds a comment '#R(auto): Line of code is too long. Break it up!' on the line before
"""
def break_up_long_lines(in_file, out_file):
    lines = in_file.readlines()
    line_number = 0
    while line_number < len(lines):
        if len(lines[line_number]) > 79:
            lines.insert((line_number - 1), '#R(auto): Line of code is too long. Break it up!')
            line_number += 1
        line_number += 1  
...
def main():
    ...
    file_name_BULL_in = 'Text files/break_up_long_lines_in.py'
    file_name_BULL_out = 'Text files/break_up_long_lines_out.py'
    with open(file_name_BULL_in, 'r') as in_file:
        with open(file_name_BULL_out, 'w') as out_file:
