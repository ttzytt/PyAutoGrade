...
"""
INSTRUCTION
The game is like tictactoe. But there are 2 sizes (size 1 and 2).
The game will ask you for a move (1A, 2A, 3B, etc.) and a size (size 1 or 2).
If you play a size 2, it is able to replace anything that is size 1.
The catch is you only have 3 big pieces that you can use. In addition a big
piece can't replace a big piece.
...
FOR EXAMPLE
    ...
    3 │ x │ o │   │ 
      └───┴───┴───┘ 
        A   B   C
If the board is like this and it is o players turn, then o player can play 2A and select size 2.
This will replace the size 1 x with a size 2 O
"""
