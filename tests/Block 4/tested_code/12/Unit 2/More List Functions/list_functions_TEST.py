...
def move_item_to_end_TEST():
    ...
    TEST("test if my_list changed", [1,3,2], my_list)

    
    my_list = [1,2,3]
    TEST("tests if it returns", None, move_item_to_end(my_list, 2))
    TEST("test if my_list changed", [1,2,3], my_list)

    
    my_list = [1]
    TEST("tests if it returns", None, move_item_to_end(my_list, 0))
    TEST("test if my_list changed", [1], my_list)
