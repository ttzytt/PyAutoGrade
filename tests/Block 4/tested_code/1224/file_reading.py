...
def longest_word(read_file):
    ...
    for i in range(len(longest_word_list)):
        longest_word_list[i] = longest_word_list[i].lower()

        if len(longest_word_list[i]) == longest_word_len:
            if longest_word_list[i] not in longerest_word_list:
                longerest_word_list.append(longest_word_list[i])
            
    return longerest_word_list
...
def longest_palindrome(read_file):
    ...
    for i in range(len(long_palindrome_list)):
        long_palindrome_list[i] = long_palindrome_list[i].lower()
        if len(long_palindrome_list[i]) == longest_palindrome_len:
            if long_palindrome_list[i] not in longest_palindrome_list:
...
def count_long_lines(read_file, min_length):
    ...
    for line in read_file:
        ...
        if len(line) > min_length:
            amount_greater += 1
    return amount_greater - num_new_line * 2
...
def random_word(read_file):
    ...
    # Generates a random number and returns the word corresponding with that number
