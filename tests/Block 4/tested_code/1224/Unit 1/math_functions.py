...
def factorial(user_number):
    ...
    while factcounter > 1:
        ...
        """'factcounter' is being used as a counter which decreases when
...
def new_triangle_number(user_number):
    ...
    if user_number %2 == 0:
        ...
        counter = 1
        """ Counter is equal to one since the number
        is odd because you have to start counting up from 1. """
        while new_triangle_number <= user_number:
            new_triangle_number = new_triangle_number + counter
            counter = counter + 2
